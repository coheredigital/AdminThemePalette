...
/**
 * AdminThemePaletteHelpers.php
 *
 * Rendering helper functions for use with ProcessWire admin theme. Derived from AdminThemeDefaultHelpers.php (Copyright (C) 2014 by Ryan Cramer) for compatiblity.
 *
...
class AdminThemePaletteHelpers extends WireData {
	...
	public function renderAdminShortcuts() {
		...
		$out =
			"<div id='head_button'>" .
			"<button class='ui-button dropdown-toggle'>$label</button>" .
	...
	public function renderAdminNotices($notices, array $options = array()) {
		...
		$defaults = array(
			...
			'listMarkup' => "\n\t<div id='notices'><div class='container'><ul>{out}</ul></div></div><!--/notices-->",
			'itemMarkup' => "\n\t\t<li class='{class}'>{remove}<i class='fa fa-fw fa-{icon}'></i> {text}</li>",
			);
